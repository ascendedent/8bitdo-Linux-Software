...
The per-field writers send one short request-1 chunk at a fixed offset, wait 100 ms, then send the commit (request 6, argument `0x123`); that is the whole of `writeUltimate2Sticks` in the DLL (real body at `0x10438a20` behind the export thunk). `writeUltimate2` loops the 45-byte chunks the same way, up to 30 tries, then commits. Neither native writer touches `crc_value`: the managed `saveCurrentConfig` (and `writeFlagEnable`/`writeFlagunEnable`) call `ANSI_CRC_16_Ultimate2` on the image as read and store the result in `crc_value` before the full write. That export is a CRC-16/MODBUS (init `0xFFFF`, reflected polynomial `0xA001`, two-entry table at `0x10c4e0a8`) over all 1592 bytes, including whatever `crc_value` held. Which range the pad verifies is unknown; `tools/u2_summary.py` prints the candidates for the first real image, and `tools/u2_plan.py` reproduces both write shapes without sending.
