...
**Rumble in DInput mode over the dongle.** The 145-byte DInput descriptor is byte for byte the same in receiver 1.00, receiver 1.03 and the pad. Besides the gamepad input report `01` (15 buttons, hat, four axes, two triggers; L4 and R4 are among the 15) it declares an 8-byte output report `01` on the LED page (four 2-byte fields), the 63-byte vendor input `02` and the 63-byte vendor output `81`. The receiver does not parse output report `01` as rumble: its USB-side parser (`0x7cd0` in 1.03) recognises only the XInput frames `00 08 ..` (rumble) and `01 03 nn` (LED). What it does parse, in both builds, is `81 11 04 08 dd dd ll rr` (1.00: handler `0x4c84`, 1.03: `0x8078`): uint16 at bytes 5-6, left and right strength at 7-8, motors driven over the radio, and a stop timer armed with the uint16. The handler sits behind the same gate in both builds (mode byte `0x842f60` in 1.00, `0x843060` in 1.03, must be 2 or 3), and in 1.03 that byte is set to 2 when the host reads the 145-byte or the 42-byte report descriptor or sends an XInput LED frame, which a Linux host does at enumeration. So on receiver 1.00 in DInput mode a userspace program can rumble the pad by writing that 64-byte report `81` to the same hidraw node it reads L4/R4 from. The strength scale is the one the XInput path feeds to the same motor call (0-255). The time unit of the uint16 is not pinned down; the XInput path arms the same timer with 200 for its own auto-stop, which reads as milliseconds but is not verified. Nothing has been sent to test this.
...
## The Ultimate 2 Wireless images

Fetched 2026-09-24 the same way (update types 94 for the pad, `PID_Ultimate2` `6012`, and 95 for the receiver, `PID_Ultimate2RR` `6013`): `u2w_1.10.dat` and `u2w_1.09.dat` (145436 bytes, header flash address `0x5000`, length `0x23800`), `u2w_adapter_1.06.dat` and `u2w_adapter_1.04.dat` (40476 bytes). All four are plain Telink `KNLT` images like the 2C's, readable with the same toolchain; flash text is linked at `0x5000` plus offset here. Prompted by six tester reports (`docs/capture-log.md`), the 1.10 pad image and the 1.06 receiver image were disassembled far enough to answer how the config channel works on this pad.

**Personalities.** The pad image carries `6012` (bcdDevice 0.01) with a single HID interface and a 145-byte descriptor, and `310b` (1.14) with the same three-interface XInput layout as the 2C's `310a`. The receiver image carries `310b` (three interfaces), `6012` (bcdDevice 0.01) with a 113-byte descriptor, `6013` (2.00), the idle `3107` with the 42-byte page `0xFFA0` descriptor, and a Nintendo Pro Controller (`057e:2009`, 203-byte descriptor). Report ids per descriptor:

| Descriptor | Where | Pages | Reports | Config channel |
| --- | --- | --- | --- | --- |
| 145 bytes | pad, cable `6012` | `0x01`, `0x02`, `0x08`, `0x09` | in `01`, out `01` (LEDs), **in `02`, out `81`** | yes, inside the gamepad collection |
| 113 bytes | receiver, dongle `6012` | `0x01`, `0x02`, `0x09`, `0xff00`, `0x0f` | in `01` (34 bytes with sensors), out `05` (rumble) | no |
| 33 bytes | both, interface 2 of `310b` | `0xff7a` | in `02`, out `81` | yes |
| 42 bytes | receiver, idle `3107` | `0xffa0` | in `02`, out `81` | receiver only |

So through the dongle in DInput mode there is no report `81` at all, which is what three testers saw. On a cable in DInput mode, and on `310b` in either place, there is.

**The pad's report-`81` set** (USB dispatcher at `0xcfd8`, handlers `0xc49c`-`0xcf64`, class-`05` dispatcher `0xbb90`) is the 2C's plus one: identify `00 21 01` (version `0x6e`), `00 51 00`, `00 61 01`, `66 aa 63`, `66 aa 64`, `66 aa 70`, `00 36`, `00 38`, a new `00 3a` (reply `0x3b`), rumble `11 04 08`, and class `05` commands `c1 4 3 5 c3 8 c2 7 c4`. `00 51 00` (`0xc51c`) writes analog register `0x3c` = `0x5a` and reboots, the same sequence as the 2C receiver's `chanegBoot_Dinput`; it is V2's `Dinput.changeDinput_CMD` (`81 05 00 51 02` with byte 4 forced to 0 before sending) and puts the pad into its DInput personality.

**The section-`04` config channel exists on this pad.** After its own handlers, the USB dispatcher checks **byte 1 == `0x04`** and hands the body at byte 2 to `0xcd8c`, which copies the 16-byte header and runs a handler per request word: `0x8`, `0x7`, `0xd`, `0x13`, `0x3`, **`0x6` commit** (`0xc7fc`), **`0x1` write** (`0xc7a0`, chunk capped at 46, copies into the image in RAM), **`0x2` read** (`0xc744`, chunk capped at 45, copies out of the image), `0x14`, `0x40`, `0x41`, `0x60`, `0x61`, and a few without a fixed word. The reply builder `0xc428` writes `04`, then the request's 16-byte header with the request word moved to offset 2 and `0x0004` at offset 0, then the data at `0x12`; the length dword is echoed as received, which is why the DLL parser compares the CRC it sent. The image is `0x638` bytes (built as `199 << 3` at `0xc820` and `0xc848`) and carries the `0x20200911` enable marks at eight places. The same dispatcher is reached from the RAM-resident radio path (`0xb96`, with a path flag of 1 so replies go back over 2.4 GHz), and from USB only when a mode byte at `0x846519` or `0x84651a` is 2, which the descriptor-request code sets once a host has claimed the device.

**The frame has no size byte on this pad.** The DLL's shared writer (real body `0x1040ec60`) sends `81 04 <body>` for product ids `6009`, `2028`, `6012`, `600f`, `600b`, `2062`, and `81 <size> 04 <body>` for everything else. Our tool sent the size-byte form to the testers' pads; the pad's dispatcher tests byte 1 for `04`, so those packets were dropped without a reply. `tools/packets.py` now builds both forms from the product id.

**The receiver relays.** Its USB dispatcher (`0x752c`) runs the receiver's own handlers (identify and friends, plus a class-`06` set at `0x6208`) and then, for a packet with byte 0 `0x81` and byte 1 `0x04`, calls `0x74e4`, which is the radio forward. So the config channel works through the dongle on any personality that has report `81`: `310b`, and whatever `6013` presents. Through the dongle's DInput `6012` there is nothing to send on.

**What V2 does on a `310b`.** `PID_Xinput` is not a config target. `S_ReadKey.s_swichReport` sends `Reportstate`, class `05` command `0x0007`, the reboot; `UpdatePage.dinputBoot` sends `Dinput.boot_CMD` (`05 00 50 01`, a report-`05` command for the DInput personality) to the current device and broadcasts `81 05 00 51 00` to a list of older ids. `changeUSBUltimate` is class `05` command `0x0009`, which this pad image does not handle. The four `Dinput.version_CMD*` arrays are `81 05 00 21 01`, `05 c0 ..`, `05 05 ..`, `aa 55 03 fc 01 60 60` and `81 04 0a`, and `IDLE_CMD` is `81 04 00 01`.
